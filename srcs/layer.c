...
static inline void    layer_set_pixel_fast(t_layer *layer, int x, int y, 
    unsigned int color)
{
    unsigned int    *data;

    data = (unsigned int *)layer->addr;
    data[y * (layer->line_length / 4) + x] = color;
}
...
static inline unsigned int   layer_get_pixel_fast(t_layer *layer, int x, int y)
{
    unsigned int    *data;

    data = (unsigned int *)layer->addr;
    return (data[y * (layer->line_length / 4) + x]);
}
...
int layer_stack_add(t_layer_stack *stack, t_layer *layer)
{
    ...
    //get the max width and height on stack width on while
    ...
    while (i < stack->count)
    {
        if (stack->layers[i]->height > max.y)
            max.y = stack->layers[max.y]->height;
        if (stack->layers[i]->width > max.x)
            max.x = stack->layers[max.x]->width;
        i++;
    }
