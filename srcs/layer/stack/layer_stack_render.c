	...
	while (i >= 0)
	{
		...
		if (layer->mask)
		{
			if (!((pos < layer->width * layer->height)
					&& layer->data[pos] > 0 && (unsigned int)src.x <= layer->width))
				i--;
		}
		else if ((src.x >= 0) && (src.x < (int)layer->width)
			&& (src.y >= 0) && (src.y < (int)layer->height))
		{
	...
	while (start_y < end_y)
	{
		...
		while (x < end_x)
		{
			get_depth_layer_color(group->layers, x - group->offset_x,
				start_y - group->offset_y, &output->data[stride + x]);
			x++;
		}
		start_y++;
	}
