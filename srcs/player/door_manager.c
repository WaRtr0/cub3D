...
void	open_door(t_game *game, int dir)
{
	...
	int			check_tile;
	int			pos;
	double	move_x = sin((game->data->yaw) * M_PI / 180) * 1.5;
	double	move_y = -cos((game->data->yaw) * M_PI / 180) * 1.5;
	...
	{
		move_x = sin((game->data->yaw) * M_PI / 180) * 0.75;
		move_y = -cos((game->data->yaw) * M_PI / 180) * 0.75;
